// ═══════════════════════════════════════════════════════════
// TRADEBOT ELECTRON GUI - CORE RENDERER (OFFICIAL RELEASE)
// ═══════════════════════════════════════════════════════════
...
/**
 * Main synchronization loop. Performs targeted DOM updates based on state changes.
 */
function syncUI() {
    ...
    // Panic Button State
    syncPanicButton(s.botIsRunning, prev);
...
function syncPanicButton(isRunning, prev) {
    ...
    if (!isRunning) {
        ...
        text.textContent = "PANIC BUTTON -\nHALT ALL TRADING";
    }
    prev._isRunning = isRunning;
}
...
function syncDecisionsTable(decisions, prev) {
    ...
    decisions.forEach(d => {
        const tr = document.createElement('tr');
        tr.className = "hover:bg-cyan-500/5 border-b border-slate-700/20 transition-colors";
...
// =======================================================================
// 4. CHART CORE (Idempotent Lifecycle)
// =======================================================================
...
// =======================================================================
// 5. WEBSOCKET & DATA FLOW (State-Driven)
// =======================================================================
...
function init() {
    ...
    window.api.on('bot-status', (p) => {
        dashboardState.botIsRunning = p.running;
        syncUI();
    });
...
function setupInteractive() {
    ...
    document.getElementById('btn-panic')?.addEventListener('click', () => {
        if (!dashboardState.botIsRunning) {
            window.api.send('start-bot');
        } else {
            const isHalted = document.getElementById('btn-panic').classList.contains('bg-red-500');
            ws?.send(JSON.stringify({ type: 'command', cmd: isHalted ? 'resume' : 'halt' }));
        }
    });
...
window.profilesModule = (function () {
    ...
    function renderList() {
        ...
        Object.keys(allProfiles).forEach(name => {
            ...
            item.className = "p-3 rounded-lg hover:bg-white/5 cursor-pointer text-slate-400 text-xs font-bold";
    ...
    function renderTab(data) {
        ...
        Object.keys(data).forEach(key => {
            ...
            input.className = "bg-black/40 border border-white/10 p-1.5 text-xs text-white rounded min-w-[120px]";
