...
# 9. The Time Machine (Backtester Trinity)
> *"We have to go back."*
...
**File:** `src/tradebot_sci/simulation/backtester.py` (The Engine) + `gui/settings_dialog.py` (The Face).
...
*   It fetches candles (from IBKR or CCXT).
*   It feeds them to the **Strategy Engine** (`strategy/engine.py`).
*   It records the fake trades in a list.

So, if you change the AI logic in `engine.py`, **all three** Time Machines will reflect the change.
