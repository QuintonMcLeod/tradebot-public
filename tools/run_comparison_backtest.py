...
INITIAL_CAPITAL = 500.0
# Backtest Settings
LONG_RISK_PCT = 0.01        # 1.0% Initial Probe
SHORT_RISK_PCT = 0.01       # 1.0% Initial Probe
FIXED_RISK_DOLLARS = 0.0    # Compounding

MAX_PYRAMID_ENTRIES = 50    # Robocop Aggression

# [COMPARISON MODE] Limit to past 2 weeks
START_LIMIT = datetime(2026, 1, 7, 0, 0, tzinfo=timezone.utc)
PROFIT_BUFFER_PCT = 0.0001  # Robocop Buffer
SECONDARY_BUFFER_PCT = 0.0001 

PYRAMID_RISK_LOAD = 1.00    # Robocop Load
PYRAMID_RISK_SCALE = 1.00   # Robocop Scale

# QUANTUM SETTINGS
MIN_HTF_STRENGTH = 0.25     # Re-enable Trend Strength Check
REQUIRE_SWEEP = True        # Re-enable Sweep Requirement
MIN_MOMENTUM_PIPS = 0.0
MIN_HTF_STRENGTH_FIXED_RISK = 0.40
MIN_STOP_PCT = 0.0008
MIN_ENTRY_RANGE_PIPS = 0.0
...
def load_candles(symbol: str) -> List[Candle]:
    filepath = os.path.join(DATA_DIR, f'{symbol}_15m.json')  # Revert to 15m for stable structure
...
def resample_candles(candles: List[Candle], timeframe: str) -> List[Candle]:
    ...
    for c in candles:
        if c.timestamp.minute % minutes == 0 and current_batch:
            resampled.append(Candle(
                timestamp=current_batch[0].timestamp,
                open=current_batch[0].open,
                high=max(b.high for b in current_batch),
                low=min(b.low for b in current_batch),
                close=current_batch[-1].close,
                volume=sum(b.volume for b in current_batch)
            ))
            current_batch = []
...
def run_forex_backtest():
    ...
    profile_symbols = ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "USDCHF", "NZDUSD", "BTCUSD", "ETHUSD", "SOLUSD", "XAUUSD"]
    ...
    for i, current_time in enumerate(all_times):
        # [COMPARISON MODE] Limit to past 3 hours
        if current_time < START_LIMIT:
            continue
        if i < 100:
            continue
        ...
        for symbol, candles in all_candles.items():
            ...
            # Session Gate Check (Convert to UTC for gating)
            utc_time = current_time.astimezone(timezone.utc)
            is_allowed_session = 8 <= utc_time.hour < 21  # 08:00 - 21:00 UTC (London + NY)
            if getattr(profile, 'session_gate_enabled', False) and not is_allowed_session:
                continue

            current_bar = candles[bar_idx]
            lookback = candles[max(0, bar_idx-250):bar_idx+1]
            current_price = current_bar.close
            
            # Get current trend (All 15m for stable Robocop-style structure)
            trend_htf = infer_trend_from_swings(lookback, window=12, swing_lookback=2, min_swings=2, strength_floor=MIN_HTF_STRENGTH)
            trend_ltf = infer_trend_from_swings(lookback[-60:], window=8, swing_lookback=2, min_swings=2, strength_floor=0.25)
            # MTF matches HTF for 15m/15m logic
            mtf_candles_15m = lookback
            ...
            if pos:
                ...
                # If trade is > 12 bars old (1 hour @ 5m) and not profitable, Kill it.
                if pos.bars_held >= 12:
                    if pos.direction == "long":
            ...
            if getattr(profile, "icc_strict_mode", False):
                ntz_zone = detect_no_trade_zone(mtf_candles_15m, swing_lookback=2)
                if ntz_zone:
                    continue

            # [ANTIGRAVITY SYNC] Signal Detection (Done on 15m MTF for structure)
            sweep = detect_liquidity_sweep(mtf_candles_15m, ltf_dir, swing_lookback=2)
            indication = detect_indication(mtf_candles_15m, swing_lookback=2)
            correction = detect_correction(mtf_candles_15m, indication, swing_lookback=2)
            continuation = detect_continuation(
                mtf_candles_15m, ltf_dir, sweep, indication, correction,
    ...
    print(f"  TOTAL TRADE EXECUTIONS (Initial + Pyramids): {len(completed_trades) + total_pyramids}")
    print(f"{'='*110}")
