...
function initChart(intervalSeconds = 900) {
    ...
    candleSeries = chart.addCandlestickSeries({
        upColor: '#00ff88',     // Brighter Neon Green
        downColor: '#ff3366',   // Brighter Neon Red
        borderVisible: false,
        wickUpColor: '#00ff88',
...
function connectWebSocket() {
    ...
    function startPingLoop() {
        ...
        // Temporarily override onmessage for pong, then restore
    ...
    ws.onmessage = (event) => {
        try {
            ...
            if (msg.type === 'candle') {
                // Update Chart
                // Ensure data matches { time, open, high, low, close }
                // msg.data might need adjustment depending on backend format
...
function formatLogMessage(msg) {
    ...
    const labels = {
        ...
        'LIVE': 'text-emerald-500 font-bold',
        'HIST': 'text-slate-600'
    };
    ...
    // 2. Buy/Sell highlights
    formatted = formatted.replace(/\b(BUY|LONG)\b/gi, '<span class="text-emerald-400 font-bold">BUY</span>');
    formatted = formatted.replace(/\b(SELL|SHORT)\b/gi, '<span class="text-rose-500 font-bold">SELL</span>');

    // 3. Numbers with +/-
    formatted = formatted.replace(/(\+\d+(\.\d+)?%?)/g, '<span class="text-emerald-400">$1</span>');
    formatted = formatted.replace(/(\-\d+(\.\d+)?%?)/g, '<span class="text-rose-500">$1</span>');
...
function appendLog(level, rawMessage) {
    ...
    div.className = "font-mono text-[11px] leading-relaxed whitespace-pre-wrap py-0.5";

    const ts = new Date().toLocaleTimeString('en-US', { hour12: false });
    div.innerHTML = `<span class="text-slate-600 mr-2">[${ts}]</span> ${formatLogMessage(rawMessage)}`;
...
function addDecisionRow(symbol, action, scoreNum, reason) {
    ...
    row.innerHTML = `
        <td class="px-8 py-4 text-slate-500 text-left font-mono">${time}</td>
        <td class="px-8 py-4 font-bold text-slate-200 text-left">${symbol}</td>
        <td class="px-8 py-4 text-left">${actionHtml}</td>
        <td class="px-8 py-4 ${scoreClass} text-left font-bold">${grade}</td>
        <td class="px-8 py-4 text-slate-400 text-[11px] italic text-left">${reason}</td>
    `;
...
function parseLogLine(line) {
    // 1. Detect Decision / Structure
    if (line.includes('[STRUCTURE]') || line.includes('Decision: Decision:') || (line.includes('[DECISION]') && line.includes('|'))) {
        try {
            let content = "";
            if (line.includes('[STRUCTURE]')) {
                content = line.split('[STRUCTURE]')[1].trim();
            } else if (line.includes('Decision: Decision:')) {
                content = line.split('Decision: Decision:')[1].trim();
            } else {
                content = line.split('[DECISION]')[1].trim();
            }

            // e.g. "SOLUSD 15m | bias=neutral ... action=stand_aside ... reason=..."
            const head = content.split('|')[0].trim();
            const symbol = head.split(' ')[0];
            const body = content.split('|')[1] || content;
            ...
            // Action
            const actionMatch = body.match(/action=([^\s]+)/i) || body.match(/gate=([^\s]+)/i);
            ...
            if (scoreMatch) {
                let rawScore = parseFloat(scoreMatch[1]);
                // Handle normalized 0-1 values from selection_score
                if (rawScore <= 1.0 && line.includes('selection_score')) {
                    score = rawScore * 100;
                } else if (rawScore <= 35.0 && line.includes('/35')) {
                    score = (rawScore / 35.0) * 100;
                } else {
                    score = rawScore;
                }
            }
            ...
            if (action === "STAND_ASIDE" || action === "SWEEP") action = "HOLD";
            addDecisionRow(symbol, action, score, reason);

        } catch (e) { console.error("Failed to parse decision:", e); }
    }

    // 2. Status/Latency
    if (line.toLowerCase().includes('connected to exchange') || line.includes('Exchange ready')) {
        updateStatus("Connected", null);
    } else if (line.toLowerCase().includes('disconnected') || line.includes('Failed to scan')) {
        updateStatus("Disconnected", "--");
    }

    // 3. Profile
    if (line.includes('[PROFILE]') || line.includes('switching to') || line.includes('Loaded profile')) {
        const match = line.match(/profile[:\s]+([\w\-]+)/i) || line.match(/switching to ([\w\-]+)/i);
        if (match && statusProfile) statusProfile.textContent = match[1].toUpperCase();
    }

    // 4. AI Commentary
    if (line.includes('[COMMENTARY]') || line.includes('commentary:')) {
        const text = line.includes('[COMMENTARY]') ? line.split('[COMMENTARY]')[1].trim() : line.split('commentary:')[1].trim();
        const el = document.getElementById('commentary-content');
        if (el) {
            if (el.innerText.includes('Waiting for')) el.innerHTML = '';
            const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            el.innerHTML += `<div class="mb-4 bg-teal-500/5 p-3 rounded-xl border-l-4 border-teal-500/40 shadow-sm animate-in fade-in slide-in-from-left-2">
                <div class="flex justify-between items-center mb-1">
                    <span class="text-[9px] font-black uppercase tracking-widest text-teal-500/60">AI Insight</span>
                    <span class="text-[9px] font-mono text-slate-600">${timestamp}</span>
                </div>
                <div class="text-sm italic leading-relaxed text-slate-300">"${text.replace(/^\"|\"$/g, '')}"</div>
            </div>`;
    ...
    // 5. Holdings / State
    if (line.includes('[HOLDINGS]') || line.includes('[STATE]')) {
        try {
            if (line.includes('[HOLDINGS]')) {
                const jsonStr = line.split('[HOLDINGS]')[1].trim();
                const data = JSON.parse(jsonStr);
                updateHoldingsTable(data);
            } else {
                // Parse "[STATE] Symbol open_position: side"
                const match = line.match(/\[STATE\]\s+(\w+)\s+open_position:\s+(\w+)/);
                if (match) {
                    const symbol = match[1];
                    const side = match[2];
                    if (side !== 'none') {
                        upsertHoldingRow(symbol, side);
                    }
                }
            }
        } catch (e) { console.error("Failed to parse holdings/state", e); }
    }

    // 6. P&L / Equity
    if (line.includes('balance') || line.includes('Liquidation') || line.includes('Capital') || line.includes('PnL')) {
        const match = line.match(/(?:balance|Liquidation|Capital|Net P&L)[:\s]+\$?\s?([\d\.,\-]+)/i);
        if (match) {
...
function setupInteractiveElements() {
    ...
    const tfLabel = document.getElementById('chart-tf-label'); // Needs ID in HTML
