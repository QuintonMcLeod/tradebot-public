...
const TOOLTIPS = {
    ...
    TREND_ICHIMOKU_ENABLED: "<strong>Ichimoku Cloud [LEGACY]</strong><br><span style='color:#818cf8;font-size:10px'>Type: Structure</span><br><br>Ichimoku draws a 'cloud' on the chart made from averaged highs and lows. <strong>NOTE:</strong> This indicator is fully implemented but no active strategy uses it for entry or exit decisions. It votes in trend consensus but its signal is drowned out by 9 other indicators. Consider leaving it disabled to reduce noise.<br><br><span style='color:#22c55e'>✓ Good for:</span> A comprehensive read on market direction. Very popular with professional crypto traders. The cloud acts as a 'comfort zone' for the price.<br><span style='color:#ef4444'>✗ Heads up:</span> Needs lots of data (52 candles minimum). In fast-moving markets, the cloud can lag behind the actual price action. <strong>No active strategy consumes this signal.</strong>",
    TREND_PARABOLIC_SAR_ENABLED: "<strong>Parabolic SAR (Stop & Reverse) [LEGACY]</strong><br><span style='color:#818cf8;font-size:10px'>Type: Trend-Following</span><br><br>Parabolic SAR places dots above or below the price. <strong>NOTE:</strong> The 'Parabolic SAR Exit' strategy is actually a 3-bar swing break — it does NOT use this indicator. No active strategy consumes SAR data directly. Consider leaving it disabled.<br><br><span style='color:#22c55e'>✓ Good for:</span> Catching trend reversals quickly. The flip is a clear, unmistakable signal. Also useful as a dynamic trailing stop level.<br><span style='color:#ef4444'>✗ Heads up:</span> In sideways markets, the dots flip constantly — every flip looks like a reversal but isn't. <strong>No active strategy uses SAR for entry/exit.</strong>",
    ...
    TREND_HULL_MA_ENABLED: "<strong>Hull Moving Average (HMA) [LEGACY]</strong><br><span style='color:#818cf8;font-size:10px'>Type: Smoothed Moving Average</span><br><br>A regular moving average is like looking in the rear-view mirror — you see where the price <em>was</em>, not where it <em>is</em>. Hull MA uses a special math trick to dramatically reduce this lag. <strong>NOTE:</strong> Fully implemented but no active strategy references Hull MA for entry or exit. It votes in trend consensus but contributes little marginal signal.<br><br><span style='color:#22c55e'>✓ Good for:</span> Getting a clean, responsive trend direction without the noise. Much faster than regular moving averages while still being smooth.<br><span style='color:#ef4444'>✗ Heads up:</span> Can be <em>too</em> responsive in very choppy markets, flipping direction on minor pullbacks. <strong>No active strategy consumes this signal.</strong>",
    ...
    // PERFORMANCE_MODE_GHOST removed — it was numerology-based (boosted risk when
    // price was near a whole number). No statistical edge. Removed from safety_guard.py.
...
const STRATEGIES = {
    ...
    forex_scrape_fade: {
        name: 'Forex Scrape Fade',
        icon: 'compress',
        shortDesc: 'Fades a deep range break, then holds for the reversion',
        assetClass: "forex",
        description: "Waits for price to push decisively outside its recent 20-bar range, then takes the other side of that push, on the view that a stretched move snaps back rather than continuing. Only fires inside the 20:00 UTC hour, and only when the break is deep enough to be an overextension rather than noise. Measured over three years on fifteen pairs, fading every break returns -5.2 pips gross while fading only deep breaks turns positive, so the depth rule is the whole strategy. Holds for up to a day: a four-hour hold measured -3.19 pips per trade net of spread against +1.15 at 288 bars.",
        style: "Mean Reversion (Range Fade)",
        risk: "Medium",
        bestFor: "Forex: 20:00 UTC hour, breaks of 2+ pips",
        stats: { target: "15 pips", stop: "20 pips", minBreak: "2 pips", maxHold: "288 bars (24h)" },
        sessionProfile: []
    },
    ...
    // Read it before editing. The GUI needs FOUR registrations and this
    // comment cannot keep them in sync on its own.
    //
    //   1. Add an entry to this STRATEGIES object with name, icon,
    //      shortDesc, assetClass (forex | crypto | universal | futures),
    //      description, style, risk, bestFor, stats, sessionProfile.
    //      This ALSO populates the per-asset dropdown on the Strategy
    //      tab, which is generated by iterating Object.entries(STRATEGIES).
    //   2. Add to the strategies array in renderStrategyToolbox() below.
    //   3. Add to STRATEGY_OPTIONS in profiles_module.js (Profile Editor).
    //   4. Add an <option> to #bt-strategy-select in index.html.
    //   5. Register in strategy/engine.py STRATEGY_REGISTRY.
    //   6. Add to Meta-SCI regime groups only if it belongs in a per-bar
    //      regime selector: strategy/variants/meta_sci.py. Most do not.
    //   7. VALIDATE: python3 -m pytest tests/test_strategy_engine.py -q
    //
    // This comment previously named renderer.js and settings.js. Neither
    // holds a strategy list — STRATEGY_OPTIONS lives in profiles_module.js
    // and settings.js does not exist. Do not reintroduce those names.
    // ──────────────────────────────────────────────────────────────
    crypto_rsi_macd: {
...
function renderExitLogicTab(container) {
    ...
    const exitStrategies = [
        ...
        { id: 'parabolic_sar', label: '3-Bar Swing Break', desc: 'Exits when 3 consecutive bars close against the trade.', tooltip: 'NOT a Parabolic SAR indicator exit. This is a momentum break: if the last 3 bars all close against your position direction, the trade is declared exhausted and exited immediately. Simple but effective.' },
...
function renderPerformanceTab(container) {
    ...
    // Harmonic Ghost toggle removed — see safety_guard.py for rationale.
...
function renderStrategyToolbox(container) {
    ...
    const strategies = [
        ...
        { id: 'forex_scrape_fade', label: 'Scrape Fade', icon: 'compress', color: '#0ea5e9' },
