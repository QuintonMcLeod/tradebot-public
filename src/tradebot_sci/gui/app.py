...
def run_app(*, repo_root: Path) -> int:
    ...
    class MainWindow(QtWidgets.QMainWindow):
        def __init__(self) -> None:
            ...
            self._candles_panel = CandlesPanel(self, self._state, settings)
            ...
            log_group = QtWidgets.QGroupBox("TRADEBOT LOG (live)")
            log_layout = QtWidgets.QVBoxLayout()
            # The log panel is a QWidget that contains the text edit and its own layout.
            log_layout.addWidget(self._log_panel)
            log_group.setLayout(log_layout)
            # Determine candle group title based on provider
            provider_label = getattr(settings, "market_provider_name", "IBKR")
            is_alt = str(getattr(settings, "exchange_provider", "")).lower() == "alternative"
            status_suffix = "live" if is_alt else "delayed"
            candle_group = QtWidgets.QGroupBox(f"CANDLES ({provider_label} {status_suffix})")
            candle_layout = QtWidgets.QVBoxLayout()
            # Insert the encapsulated candles panel instead of the inline candle widget
            candle_layout.addWidget(self._candles_panel)
            candle_group.setLayout(candle_layout)

            left_split.addWidget(log_group)
            left_split.addWidget(candle_group)
            left_split.setStretchFactor(0, 1)
            left_split.setStretchFactor(1, 1)
            # Force equal sizing initial distribution if possible
            left_split.setSizes([500, 500])

            # Right: readiness + commentary + decisions
            # Use an extracted readiness table panel instead of an inline QTableWidget
            ...
            # Commentary panel encapsulates the text widget used for AI commentary.
            self._commentary_panel = CommentaryPanel(self, self._state, settings)
            # Preserve the old attribute name for backward compatibility.  The
            # ``widget`` property on the panel returns the underlying
            # ``QTextBrowser`` so code that assigns to ``self._commentary``
            # continues to operate on the actual text widget.
            self._commentary: QtWidgets.QTextBrowser = self._commentary_panel.widget

            # Extracted orders panel encapsulates the orders table.  Replace the
            # inline QTableWidget with a dedicated panel so the main window
            # focuses on high‑level orchestration.  The panel exposes the
            # underlying table via the `_table` attribute for backward
            # compatibility.
            self._orders_panel = OrdersPanel(self, self._state, settings)
            # Preserve the old attribute name so existing references to
            # ``self._orders`` continue to work.  Assign the underlying
            # table from the panel here.  The orders panel will manage
            # all updates and formatting internally.
            self._orders: QtWidgets.QTableWidget = self._orders_panel._table

            # Decisions panel encapsulates the text widget and highlighter for recent decisions.
            ...
            readiness_group = QtWidgets.QGroupBox("CURRENT TICKER")
            readiness_layout = QtWidgets.QVBoxLayout()
            readiness_layout.addWidget(self._readiness_table)
            readiness_group.setLayout(readiness_layout)

            commentary_group = QtWidgets.QGroupBox("AI COMMENTARY")
            commentary_layout = QtWidgets.QVBoxLayout()
            # Insert the commentary panel rather than the bare text edit.
            commentary_layout.addWidget(self._commentary_panel)
            commentary_group.setLayout(commentary_layout)

            # Holdings Setup (Moved to Dashboard)
            ...
            orders_group = QtWidgets.QGroupBox("HOLDINGS & ORDERS (live)")
            orders_layout = QtWidgets.QVBoxLayout()
            
            # Holdings Section
            holdings_lbl = QtWidgets.QLabel("ACTIVE POSITIONS")
            holdings_lbl.setStyleSheet("font-weight: bold; color: #888;")
            orders_layout.addWidget(holdings_lbl)
            orders_layout.addWidget(self._holdings_table, 1) # Share vertical space
            
            # Orders Section
            orders_lbl = QtWidgets.QLabel("RECENT ORDERS")
            orders_lbl.setStyleSheet("font-weight: bold; color: #888; margin-top: 5px;")
            orders_layout.addWidget(orders_lbl)
            orders_layout.addWidget(self._orders_panel, 1) # Share vertical space
            
            orders_group.setLayout(orders_layout)

            decisions_group = QtWidgets.QGroupBox("RECENT DECISIONS (live)")
            decisions_layout = QtWidgets.QVBoxLayout()
            decisions_layout.addWidget(self._decisions_panel)
            decisions_group.setLayout(decisions_layout)

            self._symbol_details_label = QtWidgets.QLabel("Select a symbol to see details")
            self._symbol_details_label.setWordWrap(True)
            self._symbol_details_label.setStyleSheet("font-family: JetBrains Mono; font-size: 10pt; color: #4cc9f0;")
            symbol_details_group = QtWidgets.QGroupBox("SYMBOL DETAILS")
            symbol_details_layout = QtWidgets.QVBoxLayout()
            symbol_details_layout.addWidget(self._symbol_details_label)
            symbol_details_group.setLayout(symbol_details_layout)

            right_split = QtWidgets.QSplitter(QtCore.Qt.Vertical)
            right_split.addWidget(readiness_group)
            right_split.addWidget(commentary_group)
            right_split.addWidget(symbol_details_group)
            right_split.addWidget(orders_group)
            right_split.addWidget(decisions_group)
            right_split.setStretchFactor(0, 2)
            right_split.setStretchFactor(1, 2)
            right_split.setStretchFactor(2, 1) # Symbol Details
            right_split.setStretchFactor(3, 3) # Give more space to Orders/Holdings group
            right_split.setStretchFactor(4, 2)

            root_split = QtWidgets.QSplitter(QtCore.Qt.Horizontal)
            ...
            self._main_tabs.addTab(root_split, "Dashboard")
            ...
            # [ANTIGRAVITY FIX] Add PnL Tracker and Holdings Status
        ...
        def _apply_theme(self, theme_key: str) -> None:
            ...
            This implementation updates the Qt palette on the main window,
            applies stylesheets, and then delegates per‑panel theme
            application to the individual panel objects.  It also persists
            the selected theme key into the QSettings store.
            """
        ...
        def _render_holdings(self) -> None:
            ...
            if (st.startswith("ibkr ok") or st.startswith("ccxt ok")) and self._ibkr_last_fetch_ts is not None:
        ...
        def _render_right(self) -> None:
            ...
            active_sym = str(self._state.active_symbol or "").strip()
            if active_sym:
                specs = None
                # 1. Try dynamic lookup via market provider
                if self._market_provider and hasattr(self._market_provider, "get_market_definition"):
                    try:
                        mdef = self._market_provider.get_market_definition(active_sym)
                        if mdef:
                            # [ANTIGRAVITY FIX] Dynamic metadata from CCXT
                            specs = {
                                "name": mdef.get("info", {}).get("name") or mdef.get("symbol", active_sym),
                                "multiplier": mdef.get("contractSize", 1.0),
                                "unit": mdef.get("base", "")
                            }
                    except Exception:
                        pass
                
                # 2. Fallback to static specs
                if not specs:
                    from tradebot_sci.market.symbols import FUTURES_CONTRACT_SPECS
                    specs = FUTURES_CONTRACT_SPECS.get(active_sym)
                
                if specs:
                    details = f"<b>{specs.get('name', active_sym)}</b><br>"
                    details += f"Multiplier: <b>{specs.get('multiplier', 1.0)}</b> {specs.get('unit', '')}<br>"
                    self._symbol_details_label.setText(details)
                else:
                    self._symbol_details_label.setText(f"Active Symbol: <b>{active_sym}</b>")
            else:
                self._symbol_details_label.setText("No active symbol")
