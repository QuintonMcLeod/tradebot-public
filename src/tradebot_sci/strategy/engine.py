...
class StrategyEngine:
    ...
    def _load_strategy_variant(self):
        ...
        ╔══════════════════════════════════════════════════════════════════╗
        ║  ADDING A STRATEGY? READ THE PROCEDURE FIRST.                  ║
        ╠══════════════════════════════════════════════════════════════════╣
        ║                                                                ║
        ║  Authoritative checklist:                                      ║
        ║      Documentation/ADDING_A_STRATEGY.md                        ║
        ║                                                                ║
        ║  Summary — but read the document, it has the details:          ║
        ║                                                                ║
        ║  1. Class in src/tradebot_sci/strategy/variants/<name>.py      ║
        ║     Extend BaseStrategy. Implement check_entry_signal() and    ║
        ║     check_exit_signal(), optionally score_signal(). There is   ║
        ║     no evaluate() or get_signal() anywhere in this codebase.   ║
        ║                                                                ║
        ║  2. One line in STRATEGY_REGISTRY below.                       ║
        ║                                                                ║
        ║  3. Meta-SCI ensemble — only if the strategy belongs in a      ║
        ║     per-bar regime selector. See the checklist in meta_sci.py. ║
        ║                                                                ║
        ║  4. GUI registration — FOUR places, all under                 ║
        ║     src/tradebot_sci/electron_gui/ (never the stale gui/ at    ║
        ║     the repo root):                                            ║
        ║       - settings_integrated.js: const STRATEGIES = { ... }     ║
        ║         (this alone also fills the System Tab dropdown, which  ║
        ║          is generated by iterating Object.entries(STRATEGIES)) ║
        ║       - settings_integrated.js: strategies array in            ║
        ║         renderStrategyToolbox()                                ║
        ║       - profiles_module.js: STRATEGY_OPTIONS array             ║
        ║       - index.html: <option> in #bt-strategy-select            ║
        ║                                                                ║
        ║  5. Validate before claiming it works:                         ║
        ║       python3 -m pytest tests/test_strategy_engine.py \\       ║
        ║                        tests/test_integration.py -q            ║
        ║     This instantiates every registry entry and is the          ║
        ║     documented guard against a typo'd module or class path.    ║
        ║                                                                ║
        ║  This block previously named evaluate()/get_signal() and the   ║
        ║  non-existent renderer.js and settings.js. It was wrong. Do    ║
        ║  not reintroduce those names.                                  ║
        ║                                                                ║
        ╚══════════════════════════════════════════════════════════════════╝
        """
        from tradebot_sci.config.models import UserConfig
    ...
    def decide(
        ...
        # Get list of active sessions for this strategy at this timestamp
        _, can_paper_trade, active_sessions = get_schedule_status(
            profile_name=self.profile.name,
            now=now_for_sched,
            settings=self.settings,
            strategy_key=self._variant_key
        )
        ...
        # [REPLAY BYPASS] PaperBroker (backtest/replay) should never be blocked by
        # session gates — the replay data may fall outside current wall-clock windows.
        is_paper = getattr(self._broker, "is_paper", False)
        if not is_paper and self.settings and hasattr(self.settings, "risk"):
        ...
        if session_gate_enabled and session_id:
            if isinstance(session_id, list):
                ...
                suffixes = [a.split(":")[-1] for a in gates["active_sessions"] if ":" in a]
                session_ok = any(sid in gates["active_sessions"] or sid in suffixes for sid in cleaned_ids)
            else:
                cleaned_id = session_id.split(":")[-1] if isinstance(session_id, str) and ":" in session_id else session_id
                session_ok = cleaned_id in gates["active_sessions"] or cleaned_id in [a.split(":")[-1] for a in gates["active_sessions"] if ":" in a]
