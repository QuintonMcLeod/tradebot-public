...
_MEASURED_SPREAD_BPS: dict | None = None


def _measured_spread_bps(symbol: str) -> float | None:
    """Per-pair spread in basis points of price, measured from quoted candles.

    The configuration carries one global spread figure, which understates the
    crosses badly: measured round trips run from 1.1 bps on USDJPY to 2.5 bps on
    NZDUSD, and a rule that trades the crosses is flattered by a uniform number.
    Generated by tools/backfill_m5_history.py plus a measurement pass; the file is
    optional, and without it the configured value is used unchanged.
    """
    global _MEASURED_SPREAD_BPS
    if _MEASURED_SPREAD_BPS is None:
        try:
            import json as _json
            from tradebot_sci.paths import DATA_DIR as _DATA_DIR
            _p = _DATA_DIR / "measured_spreads.json"
            _MEASURED_SPREAD_BPS = _json.loads(_p.read_text()) if _p.exists() else {}
        except Exception:
            _MEASURED_SPREAD_BPS = {}
    return _MEASURED_SPREAD_BPS.get((symbol or "").upper().replace("_", "").replace("/", ""))
...
class PaperBroker:
    ...
    def _get_paper_friction(self, tier_slip_bps: float = 0.0) -> tuple[float, float, float, float, bool]:
        ...
        _sym = getattr(self, "_friction_symbol", None)
        if _sym:
            _measured = _measured_spread_bps(_sym)
            if _measured is not None:
                spread_bps = _measured
    ...
    def execute_decision(self, decision: AITradeDecision) -> tuple[ExecutionResult, ExecutionOutcome]:
        # Price this fill with the measured spread for this pair.
        self._friction_symbol = getattr(decision, "symbol", None)
        ...
        if action in {"enter_long", "enter_short"}:
            ...
            self.positions[symbol] = {
                ...
                "self_managed_risk": getattr(decision, "self_managed_risk", False),
                "mfe_usd": 0.0,
                "mae_usd": 0.0,
