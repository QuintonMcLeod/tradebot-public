        ...
        try {
            ...
            if (msg.type === 'pong') {
                ...
                if (msg.symbol === currentSym) {
                    // [ANTIGRAVITY FIX] Adjust Timestamp (Server UTC -> Local EST)
                    // The user sees "Future" because chart interprets UTC as literal time.
                    // We subtract 5 hours (18000s) to shift it visually to EST.
                    const fixedData = { ...msg.data, time: msg.data.time - 18000 };
...
function addDecisionRow(symbol, action, scoreNum, reason) {
    ...
    row.innerHTML = `
        <td class="px-2 py-4 text-slate-500 text-left font-mono text-xs">${time}</td>
        <td class="px-2 py-4 font-bold text-slate-200 text-left text-xs">${symbol}</td>
        <td class="px-2 py-4 text-left text-xs">${actionHtml}</td>
        <td class="px-2 py-4 ${scoreClass} text-left font-bold text-xs">${grade}</td>
        <td class="px-2 py-4 text-slate-400 text-xs italic text-left">${reason}</td>
    `;
