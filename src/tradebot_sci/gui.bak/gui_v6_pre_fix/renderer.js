...
function addDecisionRow(symbol, action, scoreNum, reason) {
    ...
    row.innerHTML = `
        <td class="px-1 py-4 text-slate-500 text-left font-mono text-sm">${time}</td>
        <td class="px-1 py-4 font-bold text-slate-200 text-left text-sm">${symbol}</td>
        <td class="px-1 py-4 text-left text-sm">${actionHtml}</td>
        <td class="px-1 py-4 ${scoreClass} text-left font-bold text-sm text-center">${grade}</td>
        <td class="px-1 py-4 text-slate-400 text-sm italic text-left">${reason}</td>
    `;
